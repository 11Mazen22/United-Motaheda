...
function StatusBadge({ status }: { status: SMSCampaign['status'] }) {
  const map: Record<SMSCampaign['status'], string> = {
    draft:     'bg-pharmacy-canvas text-pharmacy-inkSoft dark:bg-gray-700 dark:text-gray-300',
    ...
    cancelled: 'bg-pharmacy-canvas text-pharmacy-inkSoft dark:bg-gray-800 dark:text-gray-400',
  };
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="text-base font-bold text-pharmacy-ink dark:text-white mb-4">{children}</h2>
  );
}

function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-2xl border border-pharmacy-line
  ...
  return (
    <tr
      className={`border-b border-gray-50 dark:border-slate-700 transition-colors
        ${selected ? 'bg-brand-50 dark:bg-brand-900/20' : 'hover:bg-pharmacy-canvas dark:hover:bg-slate-700/30'}`}
    >
      ...
      <td className="px-4 py-3">
        <p className="text-sm font-semibold text-pharmacy-ink dark:text-white truncate max-w-[160px]">
          {user.full_name || '—'}
        </p>
        <p className="text-xs text-gray-400 dark:text-pharmacy-inkSoft mt-0.5">{user.email ?? ''}</p>
      </td>
      <td className="px-4 py-3 text-sm text-pharmacy-inkSoft dark:text-gray-300 font-mono">
        {user.phone ?? '—'}
      </td>
      <td className="px-4 py-3 text-xs text-pharmacy-inkSoft dark:text-gray-400">
      ...
      <td className="px-4 py-3">
        {user.marketing_consent
          ...
          : <Badge label="No"  color="bg-pharmacy-canvas text-pharmacy-inkSoft dark:bg-gray-700 dark:text-gray-400" />}
      </td>
      <td className="px-4 py-3">
        <Badge
          ...
          color={user.account_status === 'Active'
            ...
            : 'bg-pharmacy-canvas text-pharmacy-inkSoft dark:bg-gray-700 dark:text-gray-400'}
        />
      </td>
      <td className="px-4 py-3 text-sm text-center text-pharmacy-inkSoft dark:text-gray-400">
...
function CampaignCreateForm({
  ...
  return (
    <Card>
      ...
      <div className="mb-4">
        <label className="block text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 mb-2 uppercase tracking-wide">
          ...
          {([100, 200] as BatchSize[]).map((s) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-sm font-bold border-2 transition-colors
                ${batchSize === s
                  ? 'border-pharmacy-primary bg-pharmacy-primary text-white'
                  : 'border-pharmacy-line dark:border-slate-600 text-pharmacy-inkSoft dark:text-gray-300 hover:border-brand-300'}`}
            >
              {s} users
            </button>
          ))}
        </div>
        <p className="text-xs text-gray-400 dark:text-pharmacy-inkSoft mt-1">
      ...
      {/* Campaign name */}
      <div className="mb-4">
        <label className="block text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 mb-1.5 uppercase tracking-wide">
        ...
        <input
          ...
          maxLength={200}
          className="w-full px-3.5 py-2.5 rounded-xl border border-pharmacy-line dark:border-slate-600
            bg-white dark:bg-slate-700 text-sm text-pharmacy-ink dark:text-white
            focus:outline-none focus:ring-2 focus:ring-pharmacy-primary/50 focus:border-transparent"
        />
      </div>
      ...
      <div className="mb-5">
        ...
        <textarea
          ...
          maxLength={480}
          className="w-full px-3.5 py-2.5 rounded-xl border border-pharmacy-line dark:border-slate-600
            bg-white dark:bg-slate-700 text-sm text-pharmacy-ink dark:text-white resize-none
            focus:outline-none focus:ring-2 focus:ring-pharmacy-primary/50 focus:border-transparent"
          dir="auto"
        />
        <div className="flex justify-between mt-1">
          <p className="text-xs text-gray-400 dark:text-pharmacy-inkSoft">
            {charCount} chars — {smsCount} SMS message{smsCount !== 1 ? 's' : ''}
          </p>
          <p className="text-xs text-gray-400 dark:text-pharmacy-inkSoft">Max 480 chars</p>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full py-3 rounded-xl bg-pharmacy-primary text-white font-bold text-sm
          hover:bg-pharmacy-primaryDark disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
      >
...
function CampaignRow({ campaign, onLaunch, onCancel, onViewAudit, launching, cancelling, progress }: CampaignRowProps) {
  ...
  return (
    <div className="border border-pharmacy-line dark:border-slate-700 rounded-2xl p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-pharmacy-ink dark:text-white truncate">{campaign.name}</p>
          <p className="text-xs text-gray-400 dark:text-pharmacy-inkSoft mt-0.5">
      ...
      {(isActive || campaign.status === 'completed') && (
        <div>
          <div className="flex justify-between text-xs text-pharmacy-inkSoft dark:text-gray-400 mb-1">
            <span>Sent: {campaign.sent_count} · Failed: {campaign.failed_count}</span>
            <span>{pct}%</span>
          </div>
          <div className="w-full h-2 rounded-full bg-pharmacy-canvas dark:bg-slate-700 overflow-hidden">
            <div
              className="h-full rounded-full bg-pharmacy-primary transition-all duration-500"
              style={{ width: `${pct}%` }}
            />
          </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {campaign.status === 'draft' && (
          <button
            ...
            disabled={launching}
            className="px-3 py-1.5 rounded-lg bg-pharmacy-primary text-white text-xs font-bold
              hover:bg-pharmacy-primaryDark disabled:opacity-40 transition-colors"
          >
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg bg-pharmacy-canvas dark:bg-slate-700 text-pharmacy-inkSoft
            dark:text-gray-300 text-xs font-medium hover:bg-pharmacy-canvas dark:hover:bg-slate-600 transition-colors"
        >
          Audit Log
        </button>
      </div>
  ...
  return (
    <div className="p-6 max-w-screen-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-pharmacy-ink dark:text-white">SMS Marketing</h1>
          <p className="text-sm text-pharmacy-inkSoft dark:text-gray-400 mt-0.5">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors
                ${activeTab === tab
                  ...
                  : 'bg-white dark:bg-slate-800 text-pharmacy-inkSoft dark:text-gray-300 border border-pharmacy-line dark:border-slate-700 hover:bg-pharmacy-canvas dark:hover:bg-slate-700'}`}
            >
            ...
            <Card>
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                <input
                  ...
                  className="flex-1 min-w-[180px] px-3.5 py-2 rounded-xl border border-pharmacy-line dark:border-slate-600
                    bg-white dark:bg-slate-700 text-sm text-pharmacy-ink dark:text-white
                    focus:outline-none focus:ring-2 focus:ring-pharmacy-primary/50"
                />
                <select
                  value={mkt.sort}
                  onChange={(e) => mkt.setSort(e.target.value as import('@/lib/api').MarketingSortKey)}
                  className="px-3 py-2 rounded-xl border border-pharmacy-line dark:border-slate-600
                    bg-white dark:bg-slate-700 text-sm text-pharmacy-inkSoft dark:text-gray-200
                    focus:outline-none focus:ring-2 focus:ring-pharmacy-primary/50"
                >
                ...
                <label className="flex items-center gap-2 px-3 py-2 rounded-xl border border-pharmacy-line
                  dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-pharmacy-inkSoft dark:text-gray-200 cursor-pointer">
                  <input
                    type="checkbox"
                ...
                <button
                  onClick={() => mkt.refetch()}
                  className="px-3 py-2 rounded-xl border border-pharmacy-line dark:border-slate-600
                    text-sm text-pharmacy-inkSoft dark:text-gray-300 hover:bg-pharmacy-canvas dark:hover:bg-slate-700 transition-colors"
                  title="Refresh"
                >
                  ↻
                </button>
              ...
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <p className="text-xs text-pharmacy-inkSoft dark:text-gray-400">
                  {mkt.totalCount.toLocaleString()} eligible users · {selected.length} selected
                  {mkt.isFetching && <span className="ml-2 text-pharmacy-primary animate-pulse">Loading…</span>}
                </p>
                <div className="flex gap-2">
                  <button
                    onClick={selectAll}
                    ...
                    className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-brand-50 dark:bg-brand-900/20
                      text-pharmacy-primaryDark dark:text-brand-400 hover:bg-brand-100 disabled:opacity-40 transition-colors"
                  >
                    Select page
                  </button>
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-pharmacy-canvas dark:bg-slate-700
                      text-pharmacy-inkSoft dark:text-gray-300 hover:bg-pharmacy-canvas disabled:opacity-40 transition-colors"
                  >
                    Clear
                  </button>
                </div>
              ...
              {/* Table */}
              <div className="overflow-x-auto rounded-xl border border-pharmacy-line dark:border-slate-700">
                <table className="min-w-full text-left">
                  <thead className="bg-pharmacy-canvas dark:bg-slate-700/50">
                    <tr>
                      <th className="px-4 py-3 w-10" />
                      <th className="px-4 py-3 text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 uppercase tracking-wide">Name</th>
                      <th className="px-4 py-3 text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 uppercase tracking-wide">Phone</th>
                      <th className="px-4 py-3 text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 uppercase tracking-wide">Registered</th>
                      <th className="px-4 py-3 text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 uppercase tracking-wide">Consent</th>
                      <th className="px-4 py-3 text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 uppercase tracking-wide">Status</th>
                      <th className="px-4 py-3 text-xs font-semibold text-pharmacy-inkSoft dark:text-gray-400 uppercase tracking-wide text-center">Orders</th>
                    </tr>
                  </thead>
                    ...
                    {mkt.isLoading ? (
                      Array.from({ length: 8 }).map((_, i) => (
                        <tr key={i}><td colSpan={7} className="px-4 py-3">
                          <div className="h-4 bg-pharmacy-canvas dark:bg-slate-700 rounded animate-pulse w-full" />
                        </td></tr>
                      ))
                    ) : mkt.isError ? (
              ...
              <div className="flex items-center justify-between mt-4 flex-wrap gap-3">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-pharmacy-inkSoft dark:text-gray-400">Per page:</span>
                  {([50, 100, 200] as MarketingPageSize[]).map((s) => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-colors
                        ${mkt.pageSize === s
                          ? 'bg-pharmacy-primary text-white'
                          : 'bg-pharmacy-canvas dark:bg-slate-700 text-pharmacy-inkSoft dark:text-gray-300 hover:bg-gray-200'}`}
                    >
                      {s}
                    </button>
                  ))}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={mkt.page <= 1}
                    className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-pharmacy-canvas dark:bg-slate-700
                      text-pharmacy-inkSoft dark:text-gray-300 disabled:opacity-40 hover:bg-gray-200 transition-colors"
                  >
                    ← Prev
                  </button>
                  <span className="text-xs text-pharmacy-inkSoft dark:text-gray-400">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-3">
            ...
            {sms.campaignsLoading ? (
              <div className="text-sm text-gray-400 dark:text-pharmacy-inkSoft animate-pulse">Loading campaigns…</div>
            ) : sms.campaigns.length === 0 ? (
              <Card>
                <p className="text-sm text-gray-400 dark:text-pharmacy-inkSoft text-center py-6">
              ...
              <Card>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className="text-xs text-gray-400 hover:text-pharmacy-inkSoft dark:hover:text-gray-200"
                  >
                    ✕ Close
                  </button>
                </div>
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {sms.auditLog.map((entry) => (
                      <div key={entry.id} className="flex gap-2 text-xs">
                        <span className="text-gray-400 dark:text-pharmacy-inkSoft whitespace-nowrap">
                          {new Date(entry.created_at).toLocaleTimeString()}
                        </span>
                        <span className="font-semibold text-pharmacy-inkSoft dark:text-gray-300">{entry.event}</span>
