...
alter table public.prescriptions
  ...
  add column if not exists submission_source text not null default 'manual'
    check (submission_source in ('manual', 'scan', 'whatsapp')),
...
comment on column public.prescriptions.submission_source is
  'manual = customer typed an Rx number in-app. scan = customer used the camera/OCR flow (photo is processed on-device only, never uploaded). whatsapp = customer chose "Send via WhatsApp"; this is a lightweight tracking placeholder, not the actual photo.';
