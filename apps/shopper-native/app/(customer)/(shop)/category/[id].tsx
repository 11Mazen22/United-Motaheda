    ...
    <View style={s.root}>
      ...
      <View style={[s.header, { paddingTop: insets.top + 6 }]}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.chipsRow, { flexDirection: flexRow(IS_RTL) }]}>
          <Pressable onPress={toggleInStock} style={[s.chip, inStockOnly ? { backgroundColor: tint, borderColor: accent } : s.chipOff]}>
            <Ionicons name={inStockOnly ? "checkmark-circle" : "cube-outline"} size={13} color={inStockOnly ? accent : kit.color.inkFaint} />
          ...
          {SORT_OPTIONS.map((opt) => {
            ...
            return (
              <Pressable key={opt.id} onPress={() => pickSort(opt.id)} style={[s.chip, active ? { backgroundColor: tint, borderColor: accent } : s.chipOff]}>
