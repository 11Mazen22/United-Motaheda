/**
 * Manual Rx-number entry.
 *
 * There is no external pharmacy lookup API (see the removed manualLookup
 * stub in src/features/prescriptions/lib/manualLookup.ts) — instead, once a
 * well-formed, non-duplicate number is entered, the user names it and saves
 * it directly to their account via Supabase (usePrescriptionMutations).
 *
 * Digit entry is a 10-box OTP-style row that scales fluidly with screen
 * width, backed by both a hidden native TextInput (real numeric keyboard,
 * cursor, paste) and a branded on-screen keypad — both write to the same
 * state so neither input path can conflict with the other.
 */
...
import { Button, Text } from "@pharmacy/ui-native";
import { flexRow, isRtl, textAlignStart } from "@/utils/layout";
import { useAuth } from "@/features/auth";
import { usePrescriptions, usePrescriptionMutations } from "@/features/prescriptions";
...
function InfoBanner({ tone, text }: InfoBannerProps): React.ReactElement {
  const { theme } = useTheme();
  ...
  return (
    <View style={s.screen}>
      <PrescriptionsHeader
        ...
        icon="keypad-outline"
        eyebrow={t("prescriptions.manualEyebrow")}
        title={t("prescriptions.manualTitle")}
        onBack={() => router.back()}
      />
      ...
      <ScrollView
        ...
        {screenState === "ready" && (
          <Animated.View entering={FadeInDown.duration(260)} style={s.nameField}>
