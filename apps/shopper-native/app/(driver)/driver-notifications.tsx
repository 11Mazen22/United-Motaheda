 ...
 * Route is named "driver-notifications" (not the bare "notifications") to
 * avoid colliding with customer's and pharmacist's own notifications
 * screens — Expo Router route groups are invisible in the resolved URL, so
 * three personas each shipping a leaf route named "notifications" would all
 * resolve to the same "/notifications" path.
 */
...
import { Screen, Text as UIText, useTheme, type NativeTheme } from "@pharmacy/ui-native";
...
const useSStyles = (theme: NativeTheme) => useMemo(() => StyleSheet.create({
  ...
  cardUnread: { backgroundColor: theme.colors.brand.primaryLight },
