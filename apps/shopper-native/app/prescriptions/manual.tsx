...
import { usePrescriptionMutations } from "@/features/prescriptions/hooks/usePrescriptionMutations";
...
function DigitDisplay({ rxNumber, reduced, onPress }: DigitDisplayProps): React.ReactElement {
  ...
  const d = React.useMemo(() => get_d(c), [c]);
  const b = React.useMemo(() => get_b(c), [c]);
  const k = React.useMemo(() => get_k(c), [c]);
...
const palette = tone === "found"
    ? { bg: c.successTint, border: c.success, icon: c.success, name: "checkmark-circle" as const }
    : tone === "not_found"
    ? { bg: c.dangerTint,  border: c.danger,  icon: c.danger,  name: "alert-circle"      as const }
    : { bg: c.warnTint,    border: c.warn,    icon: c.warn,    name: "shield-checkmark-outline" as const };
...
function get_d(c: any) { return StyleSheet.create({
  row: {
...
function get_b(c: any) { return StyleSheet.create({
  wrap: {
    gap:          12,
    padding:      14,
    borderRadius: kit.radius.lg,
...
function get_k(c: any) { return StyleSheet.create({
  grid: {
    gap: 10,
  },
  row: {
