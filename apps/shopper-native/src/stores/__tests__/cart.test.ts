...
const { validateInventory: mockValidateInventory, reserveInventory: mockReserveInventory, releaseInventory: mockReleaseInventory, commitInventory: mockCommitInventory } =

  jest.requireMock("@/features/inventory") as {

    validateInventory: jest.Mock;
...
describe("addItem", () => {
  ...
  it("fires reserveInventory for an authed online user", async () => {
    ...
    mockValidateInventory.mockResolvedValue({ available: 10 });
...
describe("ensureReservations", () => {
  ...
  it("reserves items that are missing a reservationId", async () => {
    ...
    mockValidateInventory.mockResolvedValue({ available: 2 });
  ...
  it("patches the item with the new reservationId on success", async () => {
    ...
    mockValidateInventory.mockResolvedValue({ available: 1 });
  ...
  it("returns a ReservationError with Arabic message for insufficient_stock", async () => {
    ...
    mockValidateInventory.mockResolvedValue({ available: 0 });
    ...
    expect(failures[0].message).toBe("نفذ المخزون لهذا المنتج");
  ...
  it("continues reserving other items even after one fails", async () => {
    ...
    mockValidateInventory

      .mockResolvedValueOnce({ available: 0 })

      .mockResolvedValueOnce({ available: 1 });

    mockReserveInventory.mockResolvedValueOnce({ reservation_id: "res-p2" });
...
describe("addItem — reservation failure error messages", () => {
  ...
  it("sets Arabic 'only N available' message when available>0", async () => {
    ...
    mockValidateInventory.mockResolvedValue({ available: 3 });
...
describe("addItem — race: quantity changed before reserve resolves", () => {
  ...
  it("does NOT patch reservationId if quantity changed since reserve was fired", async () => {
    ...
    mockValidateInventory.mockResolvedValue({ available: 20 });
