...
let _pricingCache: { items: CartState["items"]; promoCode: string; shippingFee: number; result: CheckoutPricing } | null = null;

export const selectPricing = (s: CartState): CheckoutPricing => {
  if (_pricingCache && _pricingCache.items === s.items && _pricingCache.promoCode === s.promoCode && _pricingCache.shippingFee === s.shippingFee) return _pricingCache.result;
  const result =
  createCheckoutPricing(itemsToLines(s.items), {
    promoCode:   s.promoCode,
    shippingFee: s.shippingFee,
  });
  _pricingCache = { items: s.items, promoCode: s.promoCode, shippingFee: s.shippingFee, result };
  return result;
};
