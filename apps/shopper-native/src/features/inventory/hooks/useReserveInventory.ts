...
export interface UseReserveInventoryReturn
  ...
  extends Omit<UseMutationResult<ReserveResponse, Error, ReserveArgs>, "mutate" | "mutateAsync"> {
...
export function useReserveInventory(): UseReserveInventoryReturn {
  ...
  const m = useMutation<ReserveResponse, Error, ReserveArgs>({
  ...
  const buildArgs = (input: UseReserveInventoryInput): ReserveArgs => ({
