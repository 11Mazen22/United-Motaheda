...
const STATUS_I18N_KEY: Record<string, string> = {
  ...
  // archived is reachable from EITHER delivered or cancelled
  // (transition_order's graph: `status IN ('delivered','cancelled') AND
  // p_next_status = 'archived'`), so it must not claim "Delivered" — a
  // cancelled-then-archived order was showing that label, which is simply
  // false for an order the customer knows was cancelled.
  // orders.statusArchived already exists and is used correctly by the
  // pharmacist app's own OrderStatusChip; this just needed to point at it
  // instead of borrowing statusDelivered's key.
