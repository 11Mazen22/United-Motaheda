...
export default function RouteSummary({ driverCoords, destCoords }: { driverCoords?: { lat: number; lng: number } | null; destCoords?: { lat: number; lng: number } | null }) {
  ...
  return (
    <View style={s.wrap}>
      <View style={s.col}>
        <UIText variant="caption" color="secondary">Distance</UIText>
        <UIText variant="card-title">{data?.distance ?? "—"}</UIText>
      </View>
      <View style={s.col}>
        <UIText variant="caption" color="secondary">ETA</UIText>
...
const s = StyleSheet.create({
  wrap: { flexDirection: flexRow(IS_RTL), justifyContent: "space-between", marginHorizontal: kit.inset.screen, marginBottom: 12, padding: 12, borderRadius: kit.radius.lg, backgroundColor: kit.color.surface, borderWidth: 1, borderColor: kit.color.line, ...kit.shadow.card },
