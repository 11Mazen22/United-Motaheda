...
import { courier } from "@pharmacy/design-tokens";
...
export interface TypographyProps extends Omit<RNTextProps, "style"> {
  scale: keyof typeof courier.type;
...
export function Typography({ scale, color = "primary", align, style, ...props }: TypographyProps) {
  const { theme, isRTL, courier: courierTokens } = useCourierTheme();
  const metric = courierTokens.type[scale];
  ...
  return (
    <RNText
      ...
      style={[
        {
          fontFamily: fontFamilyMap[metric.weight as keyof typeof fontFamilyMap],
