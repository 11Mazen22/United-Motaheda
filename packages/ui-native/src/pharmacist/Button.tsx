  ...
  return (
    <PressableScale
      ...
      style={[
        styles.button,
        {
          backgroundColor: bg,
          borderColor,
          borderRadius: ph.radius.button,
          paddingHorizontal: ph.space[4],
    ...
    >
      {loading ? (
        ...
        <>
          {icon && <View style={{ marginRight: ph.space[2] }}>{icon}</View>}
          <Typography scale="buttonMd" color={textColor} align="center">
            {label}
          </Typography>
          {iconEnd && <View style={{ marginLeft: ph.space[2] }}>{iconEnd}</View>}
        </>
      )}
    </PressableScale>
  );
