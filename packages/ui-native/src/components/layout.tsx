...
export interface EmptyStateProps { illustration?: React.ReactNode; title: string; subtitle?: string; action?: { label: string; onPress: () => void }; style?: StyleProp<ViewStyle>; }
/** Fade-in empty state with optional primary recovery action. */
export function EmptyState({ illustration, title, subtitle, action, style }: EmptyStateProps): React.ReactElement { return <Animated.View entering={FadeIn} accessibilityLabel={title} style={[styles.state, style]}>{illustration}<Text variant="h3" align="center">{title}</Text>{subtitle ? <Text color="secondary" align="center">{subtitle}</Text> : null}{action ? <Button label={action.label} onPress={action.onPress} /> : null}</Animated.View>; }
...
export function Section({ title, subtitle, actionLabel, onAction, children, style }: SectionProps): React.ReactElement { const { isRTL } = useTheme(); return <View style={[{ gap: 12 }, style]}>{title || actionLabel ? <View style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between" }}><View style={{ flex: 1 }}>{title ? <Text variant="section-head">{title}</Text> : null}{subtitle ? <Text variant="caption" color="secondary">{subtitle}</Text> : null}</View>{actionLabel ? <Button label={actionLabel} variant="ghost" size="sm" onPress={onAction ?? (() => undefined)} /> : null}</View> : null}{children}</View>; }
...
export function Row({ children, gap = 8, align = "center", justify, reverse, style }: RowProps): React.ReactElement { const { isRTL } = useTheme(); return <View style={[{ flexDirection: (isRTL !== !!reverse) ? "row-reverse" : "row", gap, alignItems: align, justifyContent: justify }, style]}>{children}</View>; }
