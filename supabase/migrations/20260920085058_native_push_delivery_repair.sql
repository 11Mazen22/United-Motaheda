...
create or replace function public.register_device_push_token(
  p_device_id text, p_push_token text, p_platform text, p_app_version text default null
)
...
begin
  ...
  select * into v_row from public.user_devices
  ...
  if v_row.id is null then
    ...
    update public.user_devices set
      user_id = v_user_id, device_id = p_device_id, push_token = p_push_token,
      platform = p_platform, app_version = p_app_version, is_active = true,
      last_seen_at = now(), updated_at = now()
    where id = v_row.id returning * into v_row;

    update public.user_devices set is_active = false, updated_at = now()
    where id <> v_row.id and (device_id = p_device_id or push_token = p_push_token);
  end if;
  return v_row;
end;
$$;
